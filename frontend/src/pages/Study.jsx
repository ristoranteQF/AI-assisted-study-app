...
export default function Study() {
  ...
  // Mirror stats into a ref so endSession always sees the latest values
  // without re-creating the cleanup on every grade.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setDeck(d);
        // Distinguish between "deck is empty" and "everything is scheduled
        // for the future" — the first deserves "add some cards", the second
        // deserves "all caught up, come back tomorrow".
  ...
  // Idempotent: ends the session exactly once with the latest stats.
  // Reads everything from refs so the function body is stable.
  const endSession = () => {
    const id = sessionRef.current;
    if (!id) return;
    sessionRef.current = null;  // null first so concurrent callers no-op
  ...
  // End on unmount (user navigates away / closes the tab).
  useEffect(() => {
    return () => endSession();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // End immediately when the queue is exhausted, so the duration doesn't
  // keep ticking while the user lingers on the "Session complete" screen.
  useEffect(() => {
  ...
  const grade = async (quality) => {
    ...
    try {
      ...
      if (quality < 3) {
        // "Again": re-queue this card a few positions ahead so it comes
        // back within roughly a minute (matching the "< 1m" hint). Cards
        // keep coming back until graded >= Hard at least once — same as Anki.
