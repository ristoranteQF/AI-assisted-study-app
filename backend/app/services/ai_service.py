"""
AI service backed by Anthropic's Claude API.

The same long system prompt is reused across every call, so it is sent with
`cache_control: ephemeral` to take advantage of prompt caching (5-minute TTL,
1024-token minimum). Requests during a study session — generating cards,
summaries, quizzes, etc. — therefore re-use the cached prefix and pay only
for the per-call user message.
"""
...
def _extract_json(text: str) -> Any:
    ...
    try:
        ...
        # Fallback: find the first balanced JSON value.
...
\"\"\""""
    ...
    def extract_insights(self, content: str) -> dict:
        """Pull keywords + important highlight passages in a single call."""
