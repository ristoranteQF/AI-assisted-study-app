...
def update_me(
    ...
    if payload.email is not None:
        ...
        if new_email != current.email:
            # Reject if any other user already has it as their primary OR pending email,
            # to prevent two users racing to claim the same address.
            clash = db.scalar(
                select(User).where(
                    User.id != current.id,
            ...
            # Stage the change; do NOT touch current.email or is_email_verified.
            # The change only takes effect when the link sent to the new address
            # is consumed via /api/auth/confirm-email-change.
            current.pending_email = new_email
            _send_email_change_email(background, current, new_email)
        else:
            # Re-submitted the same email — clear any stale pending request.
...
def cancel_pending_email(
    ...
    """Cancel a pending email change without confirming it."""
    current.pending_email = None
    db.commit()
    db.refresh(current)
    return current
