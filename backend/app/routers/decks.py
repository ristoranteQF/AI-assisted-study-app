...
def reset_deck_progress(
    ...
    """Reset every card in the deck to fresh SM-2 state (EF=2.5, interval=0,
    repetitions=0, due now). Cards themselves are kept; only the schedule
    is cleared. Useful for re-demoing or starting a deck over.
    """
    deck = _own_deck(db, deck_id, current)
    now = datetime.now(timezone.utc)
